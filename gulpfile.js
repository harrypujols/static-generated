...
gulp.task('pages', function () {
    return gulp.src(['./content/pages/*.md', '!./content/pages/404.md'])
...
gulp.task('404', function () {
    return gulp.src('!./content/pages/404.md')
        .pipe(frontmatter({property: 'page', remove: true}))
        .pipe(md())
        .pipe(applyTemplate('./dev/templates/page.html'))
...
gulp.task('feed', ['posts'], function () {
  return dummy('rss.xml')
    .pipe(applyTemplate('./dev/templates/rss.xml'))
    .pipe(gulp.dest('./build/'));
});
...
// --- Default ---
gulp.task('default', ['compass', 'js', 'cname', 'vendor', 'assets', 'pages', 'media', 'posts', 'index', 'updates', 'tags', 'feed', 'watch', 'server', '404']);
